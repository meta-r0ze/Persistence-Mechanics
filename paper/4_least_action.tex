...
Consider a structural history $\Gamma$ composed of $N$ elementary state transitions. Each transition scrambles a finite fraction of the system's distinguishable state information into the substrate's noise floor. Let $q$ be the probability that a single transition maintains causal coherence rather than losing its informational trace to the background.
...
The standard formulation postulates this condition variationally, that nature simply does minimize action, but for reasons that are unexplained. Here we derive it as the accumulated record of all trajectories that failed to minimize their dissipation. The Principle of Least Action is the record of what is left, not the rule being followed.